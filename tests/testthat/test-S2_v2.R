test_that("Paper search with paging works", {
  res <- search_papers("covid vaccination", offset = 100, limit = 3)
  is_valid <- nrow(res$data) == 3 && (res$offset == 100) && (res$total > 6e5)
  expect_true(is_valid)
})

test_that("Paper search includes a set of fields", {
  res <- search_papers("covid", fields = "url,abstract,authors")
...
test_that("Paper search for garbage term is empty", {
  res <- search_papers("totalGarbageNonsense")
  is_valid <- (res$total == 0) && (res$offset == 0) && (length(res$data) == 0)
  expect_true(is_valid)
})

test_that("Paper with authors, year and url", {
  res <- paper2("649def34f8be52c8b66281af98ae884c09aef38b", fields = "url,year,authors")
...
test_that("Paper with citations with authors", {
  ...
  res <- paper2("649def34f8be52c8b66281af98ae884c09aef38b", fields = "citations.authors")
  is_valid <- all(vapply(res$citations$authors, nrow, integer(1)) >= 0)
  expect_true(is_valid)
})

test_that("Paper with list of twentythree authors", {
  res <- paper2("649def34f8be52c8b66281af98ae884c09aef38b", details = "authors", limit = 50)
...
test_that("Paper with authors and their affiliations and papers", {
  ...
  res <- paper2("649def34f8be52c8b66281af98ae884c09aef38b",
                details = "authors",
                fields = "affiliations,papers.paperId",
                limit = 20)
...
test_that("Author with various paper fields and paging works", {
  ...
  res <- author2(1741101, details = "papers",
...
test_that("Author with various paper fields and paging works", {
  ...
  res <- author2(1741101, details = "papers", fields = "citations.authors",
                    offset = 260)
...
test_that("Paper TLDR can be returned", {
  tldr <- paper2(identifier = "649def34f8be52c8b66281af98ae884c09aef38b", fields="tldr")$tldr$text
